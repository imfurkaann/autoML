...
def result():
    ...
    df_info = read_file(os.path.join("database", "helpful_datasets", "df_info.csv"))
    data = df_info.head(250).values.tolist()
...
def train():
    ...
    for df_path in path_listesi:
        ...
        df.name = df_path.split(".")[0].replace("_", " ")
        if "original dataset" in df.name:
            continue
